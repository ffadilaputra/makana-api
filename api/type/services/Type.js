...
module.exports = {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Type.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Type._attributes)
      .filter(attribute => attribute !== Type.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Type._attributes[attribute].type));
